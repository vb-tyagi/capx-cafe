...
import { meta, site } from './src/meta.mjs';
...
const routes = [];
function metadata(route, article) {
  const m = meta[route];
  if (!m) throw new Error(`No entry in src/meta.mjs for route ${route}`);
  const url = origin + route;
  const tag = (attr, key, value) => `<meta ${attr}="${key}" content="${esc(value)}">`;
  return [
    `<title>${esc(m.title)}</title>`,
    tag('name', 'description', m.description),
    tag('name', 'robots', site.robots),
    ...
    tag('property', 'og:title', m.socialTitle),
    tag('property', 'og:description', m.socialDescription),
    tag('property', 'og:image', site.image.url),
    tag('property', 'og:image:type', site.image.type),
    tag('property', 'og:image:width', String(site.image.width)),
    tag('property', 'og:image:height', String(site.image.height)),
    tag('property', 'og:image:alt', site.image.alt),
    tag('name', 'twitter:card', 'summary_large_image'),
    tag('name', 'twitter:title', m.socialTitle),
    tag('name', 'twitter:description', m.socialDescription),
    tag('name', 'twitter:image', site.image.url),
    tag('name', 'twitter:image:alt', site.image.alt),
    ...(article ? [tag('name', 'author', 'Vaibhav Tyagi')] : []),
  ].join('\n') + '\n';
}
async function render(route, content, article) {
  let nav = header;
  ...
  const schema = article ? `<script type="application/ld+json">${JSON.stringify({
    ...
    dateModified: '2026-09-16', image: site.image.url, inLanguage: 'en',
  }).replaceAll('<', '\\u003c')}</script>` : '';
  const html = rootAssets([
    '<!doctype html>\n<html lang="en">\n<head>', head, metadata(route, article), schema,
...
await render('/', home);
await render('/pricing/', await read('pricing.html'));
...
<div class="article-layout wrap"><nav class="article-toc" aria-label="On this page"><p class="eyebrow">In this article</p>${headings.map(([,id,title]) => `<a href="#${id}">${title}</a>`).join('')}</nav><div class="article-body">${post.body}<section class="article-sources" aria-labelledby="sources-title"><h2 id="sources-title">Sources &amp; further reading</h2><ul>${post.sources.map(([label,href]) => `<li><a href="${esc(href)}">${esc(label)}</a></li>`).join('')}</ul></section><aside class="article-next"><h3>Try it with your own work.</h3><p>Choose your agent, connect capx café, and start with a preview of one real change.</p><a class="button lime" href="/#install">Set up capx café <span aria-hidden="true">↗</span></a></aside></div></div></article><section class="related-posts wrap" aria-labelledby="related-title"><h2 id="related-title">Keep reading.</h2><div class="related-grid">${related}</div></section>`;
  await render(postLink(post), body, post);
}
