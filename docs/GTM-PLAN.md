...
_Canonical asset checklist — folded in from the earlier `GTM.md` draft (now removed 2026-07-19) with
statuses updated to current reality._
...
| 0.5 | Demo GIF/video | 20s vhs-scripted cut, real chokepoint code on screen (`tools/demo/`); sandbox cut is FINAL (founder, 2026-09-01 — no real-post re-record); embedded README top + landing How-it-works; `pnpm demo` replays | ✅ done |
| 0.6 | Landing page | **LIVE at capx-cafe.vercel.app** — first cut 2026-08-29; **redesigned by the founder and shipped 2026-09-16** (10 pages: home, `/pricing/`, `/blog/` + 7 articles by Vaibhav Tyagi; nav How it works · Built-in-Safety · Pricing · Blog · GitHub ↗ · Set up CTA; built from `landing/src/` by `landing/build.mjs`, guarded by `landing/test/`). The home leads with the developer-workflow framing ("Turn what you ship into what you share"); the safety thesis is the trust line, and the XMCP comparison now lives in the README only — founder decision, listings deliberately left on the original one-liner. **2026-09-17:** per-page search + social metadata from the founder's guide (`landing/src/meta.mjs`), new share image `assets/capx-cafe-social-v3.jpg` (1200×630) on all ten pages; GitHub repo social preview + About text updated by the founder. How it's built + deploy notes: `landing/README.md`. | ✅ live |
